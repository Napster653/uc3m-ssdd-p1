...
int addNode(int k, char * v1, float v2){
    ...
    if(check != NULL) return -1;//La función devuelve -1 (ERROR) si la clave ya fue registrada
    Node new = createNode(k, v1, v2);
    if(size > 0) new->next = head;//La función solo asigna el puntero NEXT si hay algún nodo en la lista
    head = new;
    size++;
    return 1;
}
...
int removeNode(int k){
    Node check = getNode(k);
    if(check == NULL) return -1;//La función devuelve -1(ERROR) si no encuentra la clave k en la lista
    //Para eliminar, el nodo es ignorado dentro de la lista y su espacio es liberado (free)
    if(check == head) head = head->next;//Si es el primer elem, solo se cambia el puntero de head al segundo         
    else{
...
int edit(int k, char * v1, float v2){
    Node elem = getNode(k);
    if(elem == NULL) return -1;//Si la clave no existe (ERROR), la función devuelve -1
    elem->value1 = v1;
    elem->value2 = v2;
    return 1;
}
